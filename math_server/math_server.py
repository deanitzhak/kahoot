...
import json
import time
from math_game import MathQuizGame  # Ensure this matches your game class name
...
class MathServer:
    ...
    def start(self):
        ...
        gui_server.math_server = self
        print("GUI server started on port 5001")
        gui_server.serve_forever()

# Inner class to handle HTTP requests from the GUI or client
class GUIRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/get_status':
            ...
            with self.server.math_server.lock:
                status = []
                for player_id, player in self.server.math_server.quiz_game.players.items():
                    status.append({
                        'player_id': player_id,
                        'score': self.server.math_server.quiz_game.get_player_score(player_id),
